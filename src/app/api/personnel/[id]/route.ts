...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) { try { const { id } = personnelIdSchema.parse(await params); return NextResponse.json({ success: true, data: await personnelService.getById(id, getCurrentTenantId()) }); } catch (error) { return handleApiError(error); } }
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) { try { const { id } = await params; return NextResponse.json({ success: true, data: await personnelService.update(id, getCurrentTenantId(), updatePersonnelSchema.parse({ ...(await request.json()), id })) }); } catch (error) { return handleApiError(error); } }
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) { try { const { id } = personnelIdSchema.parse(await params); await personnelService.softDelete(id, getCurrentTenantId()); return NextResponse.json({ success: true, data: { deleted: true } }); } catch (error) { return handleApiError(error); } }
