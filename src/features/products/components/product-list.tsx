...
export default function ProductList() {
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      try {
        const response = await fetch(`/api/products?pageSize=100${search ? `&search=${encodeURIComponent(search)}` : ""}`, { signal: controller.signal });
  ...
  return <div className="space-y-6">
    ...
    <div className="relative max-w-md"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Kod, ürün adı veya kategori ara..." className="pl-9" /></div>
