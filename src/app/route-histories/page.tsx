  ...
  return <DashboardLayout>
    <div className="space-y-6 sm:space-y-8">
      ...
      {histories.length === 0 ? <div className="rounded-2xl border border-dashed border-border p-12 text-center"><MapPinned className="mx-auto size-9 text-muted-foreground" /><p className="mt-3 font-medium">Henüz kayıtlı rota yok</p><p className="mt-1 text-sm text-muted-foreground">Dashboard’da rota oluşturulduğunda günlük kayıt otomatik tutulur.</p></div> : <div className="space-y-4">{histories.map((history) => {
        ...
        return <article key={history.id} className="rounded-2xl border border-border/70 bg-card p-5 shadow-sm sm:p-6"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-sm font-medium text-primary">{new Date(history.routeDate).toLocaleDateString("tr-TR", { dateStyle: "full", timeZone: "UTC" })}</p><h2 className="mt-1 text-xl font-bold">{history.orderCount} teslimat noktası</h2><p className="mt-1 text-sm text-muted-foreground">{stops.map((stop) => stop.customerName || stop.orderCode).filter(Boolean).join(" → ") || "Durak bilgisi yok"}</p></div>{googleUrl && <Button size="sm" variant="outline" render={<a href={googleUrl} target="_blank" rel="noreferrer" />}><MapPinned className="size-4" /> Son durağı aç</Button>}</div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Stat label="Mesafe" value={`${history.distanceKm.toLocaleString("tr-TR", { maximumFractionDigits: 1 })} km`} /><Stat label="Tahmini yakıt" value={`${history.estimatedFuelLiters.toLocaleString("tr-TR", { maximumFractionDigits: 1 })} L`} /><Stat label="Yakıt fiyatı" value={`${money(history.fuelPricePerLiter)} / L`} /><Stat label="Yakıt maliyeti" value={money(history.estimatedFuelCost)} /><Stat label="Teslim ciro" value={money(history.revenue)} /></div></article>;
      })}</div>}
    </div>
  </DashboardLayout>;
}
