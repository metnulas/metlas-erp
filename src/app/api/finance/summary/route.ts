...
export async function GET(request: NextRequest) {
  try {
    ...
    const [payments, expenses, dueEntries] = await Promise.all([
      prisma.salesPayment.findMany({ where: { tenantId, paidAt: { gte: start, lte: end } }, select: { id: true, amount: true, method: true, paidAt: true, referenceNumber: true, order: { select: { orderCode: true } }, customer: { select: { fullName: true } }, partner: { select: { id: true, name: true } } }, orderBy: { paidAt: "asc" } }),
      prisma.expense.findMany({ where: { tenantId, expenseDate: { gte: new Date(`${from}T00:00:00.000Z`), lte: new Date(`${to}T23:59:59.999Z`) } }, select: { amount: true, type: true, paymentMethod: true } }),
