...
export default function UserRoleEditor({ user, roles }: { user: User; roles: Role[] }) {
  ...
  return <div className={`flex flex-col gap-4 rounded-xl border border-border/70 p-4 ${!user.isActive ? "opacity-60" : ""}`}><div className="grid gap-2 sm:grid-cols-2"><Input aria-label="Kullanıcı adı" value={name} onChange={(event) => setName(event.target.value)} /><Input aria-label="Telefon" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder={user.email} /></div><p className="text-xs text-muted-foreground">{user.email} · {user.isActive ? "Aktif" : "Pasif"}</p><div className="flex flex-wrap items-center gap-2">{roles.map((role) => <label key={role.id} className="flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs"><input type="checkbox" checked={selected.has(role.id)} onChange={() => setSelected((current) => { const next = new Set(current); if (next.has(role.id)) next.delete(role.id); else next.add(role.id); return next; })} />{role.name}</label>)}<Button size="xs" onClick={save} disabled={saving}>{saving ? "..." : "Rolleri kaydet"}</Button><Button size="xs" variant="outline" onClick={updateProfile} disabled={saving}>Düzenle</Button><Button size="xs" variant="outline" onClick={toggleActive}><span className="sr-only">Durum: </span>{user.isActive ? <UserRoundX className="size-3.5" /> : <UserCheck className="size-3.5" />}{user.isActive ? "Pasifleştir" : "Aktifleştir"}</Button><Button size="xs" variant="outline" onClick={resetPassword}><KeyRound className="size-3.5" /> Şifre</Button><Button size="xs" variant="destructive" onClick={deleteUser}><Trash2 className="size-3.5" /> Sil</Button></div></div>;
}
