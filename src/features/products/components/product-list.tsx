...
export default function ProductList() {
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      try {
         setIsLoading(true);
         const response = await fetch(`/api/products?page=${page}&pageSize=20${debouncedSearch ? `&search=${encodeURIComponent(debouncedSearch)}` : ""}`, { signal: controller.signal });
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
     ...
     <div className="relative max-w-md"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="Kod, ürün adı veya kategori ara..." className="pl-9" /></div>
