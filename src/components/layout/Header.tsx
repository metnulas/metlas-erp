...
import GlobalSearch from "@/features/platform/components/GlobalSearch";
  ...
  return (
    <header className="sticky top-0 z-30 flex h-[4.5rem] shrink-0 items-center gap-3 border-b border-slate-200/80 bg-white/80 px-4 shadow-[0_14px_35px_-28px_rgba(15,23,42,0.55)] backdrop-blur-2xl sm:px-6 lg:px-10">
      <Button aria-label="Menüyü aç" className="lg:hidden" onClick={onMenuClick} size="icon" variant="ghost"><Menu /></Button>
        {session?.user?.isGlobalAdmin ? <GlobalSearch /> : <div className="relative hidden max-w-xl flex-1 sm:block"><Input aria-label="Genel arama" className="h-11 rounded-xl border-slate-200/70 bg-slate-100/75 pl-4 shadow-none" placeholder="Müşteri, sipariş veya araç ara..." /></div>}
