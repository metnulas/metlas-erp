...
async function validatePermissionIds(permissionIds: string[], tenantId: string | null) {
  const permissions = await prisma.permission.findMany({ where: { id: { in: permissionIds }, deletedAt: null, isActive: true, OR: [{ tenantId: null }, ...(tenantId ? [{ tenantId }] : [])] }, select: { id: true } });
...
export async function createUser(tenantId: string | null, input: { name: string; email: string; password: string; roleIds: string[] }, actorId: string) {
  ...
  const user = await prisma.$transaction(async (tx) => {
    const created = await tx.user.create({ data: { tenantId, email: input.email, name: input.name, passwordHash: await hashPassword(input.password), role: "OPERATIONS" } });
  ...
  return { id: user.id, name: user.name, email: user.email, roleIds: input.roleIds };
}
