...
powershell -NoProfile -Command "$esc=[char]27; $ip='!SRV_IP!'; if([string]::IsNullOrWhiteSpace($ip)){$ip='127.0.0.1'}; Write-Host '  Local :' -NoNewline; Write-Host \"$esc]8;;http://localhost:3000$([char]7)http://localhost:3000$esc]8;;$([char]7)\"; Write-Host '  LAN   :' -NoNewline; Write-Host \"$esc]8;;http://$ip:3000$([char]7)http://$ip:3000$esc]8;;$([char]7)\""
echo.
