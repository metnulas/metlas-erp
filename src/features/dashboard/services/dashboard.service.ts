...
export interface DashboardSummary {
  ...
  partnerOrders: Array<{ id: string; name: string; code: string; orderCount: number }>;
}
  ...
  return {
    async getSummary(tenantId: string): Promise<DashboardSummary> {
      ...
      return {
        ...
        criticalStock: orders[10], recentOrders: orders[11].map((order) => ({ ...order, grandTotal: Number(order.grandTotal), customerName: order.customer.fullName })), routeOrders: orders[12], partnerOrders: orders[13],
      };
    },
  };
}
