...
export async function GET(request: NextRequest) {
  try {
    ...
    const [payments, expenses, dueEntries] = await Promise.all([
      prisma.salesPayment.findMany({ where: { tenantId, paidAt: { gte: start, lte: end } }, select: { id: true, amount: true, method: true, paidAt: true, referenceNumber: true, order: { select: { orderCode: true } }, customer: { select: { fullName: true } }, partner: { select: { name: true } } }, orderBy: { paidAt: "asc" } }),
